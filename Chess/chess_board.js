...
let BLOCK_COLOUR_1 = '#9f7119',
	BLOCK_COLOUR_2 = '#debf83',
	HIGHLIGHT_COLOUR = '#fb0006';

let piecePositions = null;
...
function blockOccupiedByEnemy(clickedBlock) {
	let team = (currentTurn === BLACK_TEAM ? json.white : json.black);
...
function blockOccupied(clickedBlock) {
	let pieceAtBlock = getPieceAtBlockForTeam(json.black, clickedBlock);
...
function getPieceAtBlock(clickedBlock) {
	...
	let team = (currentTurn === BLACK_TEAM ? json.black : json.white);
...
function drawPiece(curPiece, bBlackTeam) {
	...
	let imageCoords = getImageCoords(curPiece.piece, bBlackTeam);
...
function checkIfPieceClicked(clickedBlock) {
	let pieceAtBlock = getPieceAtBlock(clickedBlock);
...
function movePiece(clickedBlock, enemyPiece) {
	...
	let team = (currentTurn === WHITE_TEAM ? json.white : json.black),
